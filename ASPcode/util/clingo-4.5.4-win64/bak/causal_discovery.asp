...
%dconn(X,Y,context(Z,E)) :- outside_set(X,Z), outside_set(Y,Z), into(X,Y,context(Z,E)), X < Y.
%dconn(X,Y,context(Z,E)) :- outside_set(X,Z), outside_set(Y,Z), outof(X,Y,context(Z,E)), X < Y.

dconn(X,Y,context(Z,E)) :- outside_set(X,Z), outside_set(Y,Z), into(X,Y,context(Z,E)).
dconn(X,Y,context(Z,E)) :- outside_set(X,Z), outside_set(Y,Z), outof(X,Y,context(Z,E)).

%dconn(X,Y,C) :- into(X,Y,C).%, X < Y.
%dconn(X,Y,C) :- outof(X,Y,C).%, X < Y.
dconn(Y,X,C) :- dconn(X,Y,C).

noncollideropen(X,Z) :- outside_set(X,Z), condset(Z).
collideropen(X,Z) :- inside_set(X,Z), condset(Z).

into(X,Y,context(Z,E)) :- notmanipulated(Y,E), head(X,Y), noncollideropen(X,Z). % Case 0a
outof(X,Y,context(Z,E)) :- notmanipulated(X,E), arrow(Y,X), noncollideropen(Y,Z). % Case 0b
into(X,W,context(Z,E)) :- into(X,Y,context(Z,E)), noncollideropen(Y,Z), arrow(Y,W), notmanipulated(W,E).%, X < W. % Case 1
outof(X,W,context(Z,E)) :- into(X,Y,context(Z,E)), collideropen(Y,Z), notmanipulated(Y,E), arrow(W,Y).%, X < W. % Case 2
into(X,W,context(Z,E)) :- outof(X,Y,context(Z,E)), noncollideropen(Y,Z), head(Y,W), notmanipulated(W,E).%, X < W. % Case 3
...
%into(X,W,context(Z,E)) :- into(X,Y,context(Z,E)), collideropen(Y,Z), notmanipulated(W,E), head(W,Y), head(Y,W).%, X < W. % Case 2a
into(X,W,context(Z,E)) :- into(X,Y,context(Z,E)), collideropen(Y,Z), notmanipulated(W,E), latentedge(Y,W).%, X < W. % Case 2a
into(X,W,context(Z,E)) :- into(X,Y,context(Z,E)), collideropen(Y,Z), notmanipulated(W,E), latentedge(W,Y).%, X < W. % Case 2a
#show latentedge/2.
...
%outof(X,Y,context(Z,E)) :- context(Z,E), selection(X), selection(Y), noncollideropen(X,Z), noncollideropen(Y,Z), X != Y. % Case 0c
