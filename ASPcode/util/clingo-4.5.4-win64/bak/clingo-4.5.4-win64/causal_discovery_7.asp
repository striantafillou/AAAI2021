...
dconn(X,Y,C) :- hh(X,Y,C), X < Y.
dconn(X,Y,C) :- ht(X,Y,C), X < Y.
dconn(X,Y,C) :- th(X,Y,C), X < Y.
dconn(X,Y,C) :- tt(X,Y,C), X < Y.

%noncollideropen(X,Z) :- outside_set(X,Z), condset(Z).
%collideropen(X,Z) :- inside_set(X,Z), condset(Z).

th(X,Y,context(Z,E)) :- condset(Z), expset(E), arrow(X,Y), outside_set(X,Z), X < Y.
ht(X,Y,context(Z,E)) :- condset(Z), expset(E), arrow(Y,X), outside_set(Y,Z), X < Y.

hh(X,W,context(Z,E)) :- hh(X,Y,context(Z,E)), arrow(Y,W), outside_set(Y,Z), X != Y, Y != W, X < W.
hh(X,W,context(Z,E)) :- ht(X,Y,context(Z,E)), arrow(Y,W), outside_set(Y,Z), X != Y, Y != W, X < W.
th(X,W,context(Z,E)) :- th(X,Y,context(Z,E)), arrow(Y,W), outside_set(Y,Z), X != Y, Y != W, X < W.
th(X,W,context(Z,E)) :- tt(X,Y,context(Z,E)), arrow(Y,W), outside_set(Y,Z), X != Y, Y != W, X < W.

ht(X,W,context(Z,E)) :- hh(X,Y,context(Z,E)), arrow(W,Y), inside_set(Y,Z), X != Y, Y != W, X < W.
ht(X,W,context(Z,E)) :- ht(X,Y,context(Z,E)), arrow(W,Y), outside_set(Y,Z), X != Y, Y != W, X < W.
tt(X,W,context(Z,E)) :- th(X,Y,context(Z,E)), arrow(W,Y), inside_set(Y,Z), X != Y, Y != W, X < W.
tt(X,W,context(Z,E)) :- tt(X,Y,context(Z,E)), arrow(W,Y), outside_set(Y,Z), X != Y, Y != W, X < W.

%into(X,Y,context(Z,E)) :- condset(Z), expset(E), not manipulated(Y,E), head(X,Y), noncollideropen(X,Z), X != Y. % Case 0a
%outof(X,Y,context(Z,E)) :- condset(Z), expset(E), arrow(Y,X), noncollideropen(Y,Z), X != Y.  % Case 0b
%into(X,C,context(Z,E)) :- into(X,Y,context(Z,E)), noncollideropen(Y,Z), arrow(Y,C), X != Y, Y != C.%, X < C. % Case 1
%outof(X,P,context(Z,E)) :- into(X,Y,context(Z,E)), collideropen(Y,Z), not manipulated(Y,E), arrow(P,Y), X != Y, Y != P.%, X < P. % Case 2
%into(X,C,context(Z,E)) :- outof(X,Y,context(Z,E)), noncollideropen(Y,Z), head(Y,C), X != Y, Y != C.%, X < C. % Case 3
%outof(X,P,context(Z,E)) :- outof(X,Y,context(Z,E)), noncollideropen(Y,Z), not manipulated(Y,E), arrow(P,Y), X != Y, Y != P.%, X < P. % Case 4
